...
DEFAULT_OUT_ROOT = "./eval_M0_M7/snapshots"
DEFAULT_DEVICE = "cpu"
DEFAULT_GPU_ID = 0
...
DEFAULT_SATURATION_ERR_ABS = 0.25
...
def get_arrays_for_time(arrays: dict[str, Any], time_value: float) -> dict[str, np.ndarray]:
    x = to_np(arrays["x"]).reshape(-1).astype(np.float64)
    y = to_np(arrays["y"]).reshape(-1).astype(np.float64)
    t = to_np(arrays["t"]).reshape(-1).astype(np.float64)
    p = to_np(arrays["p"]).reshape(-1).astype(np.float64)
    s = to_np(arrays["Sco2"]).reshape(-1).astype(np.float64)
    ...
    return {"x": x[idx], "y": y[idx], "p": p[idx], "S": np.clip(s[idx], 0.0, 1.0)}
...
def interpolate_to_grid(x: np.ndarray, y: np.ndarray, values: np.ndarray, X: np.ndarray, Y: np.ndarray, mask: np.ndarray):
    ...
    tri = mtri.Triangulation(x[good], y[good])
    interp = mtri.LinearTriInterpolator(tri, vals[good])
    Z = np.asarray(interp(X, Y), dtype=np.float64)
    # Fill remaining holes by nearest-neighbor only inside the physical square, then re-mask wells.
    if np.isnan(Z).any():
        try:
            from scipy.interpolate import NearestNDInterpolator
            nn = NearestNDInterpolator(np.column_stack([x[good], y[good]]), vals[good])
...
def export_run(run: RunSpec, label: str, arrays: dict[str, Any], time_unique: np.ndarray, args):
    ...
    if str(args.device).lower() == "cuda" and torch.cuda.is_available():
...
def main():
    ...
    if args.ckpt:
        ...
        runs.append((RunSpec(f"{exp}_{st}_LOO-{lo}_seed{sd}", exp, st, lo, sd, args.ckpt), label))
    else:
        from evaluate_global_accuracy_M0_M7_L6_dS_shortlabels import parse_run_from_filename
