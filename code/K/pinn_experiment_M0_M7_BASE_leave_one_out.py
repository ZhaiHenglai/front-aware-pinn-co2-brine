...
from configuration_k import (
    ...
    FV_DT_TILDE as K_FV_DT_TILDE,
    LEARNING_RATE,
    NETWORK_WIDTH,
    RAR_CANDIDATE_COUNT,
    RAR_EVERY as K_RAR_EVERY,
    RAR_GLOBAL_SELECT_COUNT,
    RAR_RESIDUAL_SELECT_COUNT,
    RAR_START as K_RAR_START,
    T_REF_S,
...
from eos_k import fit_eos_record, torch_eos_from_record
...
from permeability_field_k import PermeabilityFieldK, file_sha256, validate_asset_bundle
...
RUN_KIND = os.environ.get("PINN_RUN_KIND", "formal").strip().lower()
MODEL_ROLE = os.environ.get("PINN_MODEL_ROLE", "complete").strip()
TASK_SPEC = validate_task(MODEL_ROLE, EXP_NAME, TRAINING_STRATEGY, LEAVE_OUT, SEED)

# Configuration K is a no-capillary-pressure study.  The retained 723 trainer
# has no Pc term, so these values are both a fail-closed input contract and an
# explicit checkpoint record of the actual equations being solved.
PC_ENABLE = os.environ.get("PINN_PC_ENABLE", "0").strip().lower() in {"1", "true", "yes", "on"}
PC_ENTRY_PRESSURE = float(os.environ.get("PINN_PC_ENTRY", "0"))
validate_no_pc(os.environ)
if os.environ.get("PINN_DATA_HOLDOUT", "0").strip().lower() in {"1", "true", "yes", "on"}:
    raise ValueError("Configuration K formal/smoke workflow does not permit data holdout training.")

T_REF_REQUESTED = float(os.environ.get("PINN_T_REF", str(T_REF_S)))
ANALYSIS_T_MAX_REQUESTED = float(os.environ.get("PINN_ANALYSIS_T_MAX", str(ANALYSIS_T_MAX_S)))
if T_REF_REQUESTED != T_REF_S:
    raise ValueError(f"Configuration K requires PINN_T_REF={T_REF_S}; got {T_REF_REQUESTED}.")
if ANALYSIS_T_MAX_REQUESTED != ANALYSIS_T_MAX_S:
    raise ValueError(
        f"Configuration K requires PINN_ANALYSIS_T_MAX={ANALYSIS_T_MAX_S}; "
        f"got {ANALYSIS_T_MAX_REQUESTED}."
    )

PROJECT_ROOT = Path(__file__).resolve().parent
RUN_DIR_RAW = os.environ.get("PINN_RUN_DIR", "").strip()
if not RUN_DIR_RAW:
    raise ValueError("PINN_RUN_DIR is required so every Configuration K task has an isolated output directory.")
RUN_DIR = Path(RUN_DIR_RAW)
DATASET_PATH = Path(os.environ.get("PINN_DATA_PT", str(PROJECT_ROOT / DATASET_RELATIVE_PATH)))
K_FIELD_PATH = Path(os.environ.get("PINN_K_FIELD_NPZ", str(PROJECT_ROOT / "permeability/K_field_unique_grid.npz")))
K_FIELD_METADATA_PATH = Path(
    os.environ.get("PINN_K_FIELD_METADATA", str(PROJECT_ROOT / "permeability/K_field_unique_grid.json"))
)
REFERENCE_PROVENANCE_PATH = Path(
    os.environ.get("PINN_REFERENCE_PROVENANCE", str(PROJECT_ROOT / "manifests/k_reference_provenance.json"))
)
os.environ["PINN_DATA_PT"] = str(DATASET_PATH)

K_ASSET_REPORT = validate_asset_bundle(
    DATASET_PATH,
    K_FIELD_PATH,
    K_FIELD_METADATA_PATH,
    REFERENCE_PROVENANCE_PATH,
    run_kind=RUN_KIND,
)

# put these toggles here, as requested
FLOAT_DTYPE = "float64"          # frozen to match Configuration H
...
FINITE_CHECK_EVERY = 10          # fail-fast health check; does not change the optimizer or loss
...
USE_DATA_MAX_TIME_AS_T_REF = False
T_REF_FALLBACK = T_REF_S
...
# dataset
# Configuration K has one immutable reference dataset.  Asset preflight above
# validates its SHA-256 and exports the exact path; do not search legacy account
# locations or accept an ambiguously named fallback cache.
DATA_PT_CANDIDATES = [str(DATASET_PATH)]
...
MAX_ADAM_ITERS = int(
    os.environ.get(
        "PINN_MAX_ADAM_ITERS",
        str(FORMAL_MAX_ADAM_ITERS if RUN_KIND == "formal" else 100),
    )
)

# Residual-biased resampling controls.  RAR-style resampling is enabled only from M6 onward.
RAR_START = K_RAR_START
RAR_EVERY = K_RAR_EVERY
...
def _detect_runtime_config():
    ...
    if not torch.cuda.is_available():
        base.update({
            ...
            "N_CAND": 10000,
            "N_SEL_R": 7500,
            "N_SEL_G": RAR_GLOBAL_SELECT_COUNT,
            "DATA_BATCH": 4096,
            "USE_LBFGS": False,
            "LBFGS_MAX_ITER": 0,
...
# Freeze the Configuration H/V100 numerical budget even if Maxwell schedules a
# different GPU model.  Hardware identity is recorded separately; it must not
# silently change model capacity or RAR sampling.
RUNTIME.update({
    "profile": "v100",
    "width": NETWORK_WIDTH,
    "N_BASE": 38000,
    "N_POOL": 120000,
    "N_CAND": RAR_CANDIDATE_COUNT,
    "N_SEL_R": RAR_RESIDUAL_SELECT_COUNT,
    "N_SEL_G": RAR_GLOBAL_SELECT_COUNT,
    "DATA_BATCH": 24576,
    "USE_LBFGS": False,
    "LBFGS_MAX_ITER": 0,
...
L_ref = 5.0
T_ref = float(T_REF_S)
# Frozen H reference scale. Pointwise physics uses PERM_FIELD(x,y); this scalar
# is only the nondimensionalisation reference and is not a homogeneous K field.
K = 1.0e-14
phi = 0.2
...
PERM_FIELD = PermeabilityFieldK.from_npz(K_FIELD_PATH).to(device=DEVICE, dtype=DTYPE)
_K_GRID_MIN = float(torch.exp(PERM_FIELD.log_k_flat.min()).detach().cpu().item())
_K_GRID_MAX = float(torch.exp(PERM_FIELD.log_k_flat.max()).detach().cpu().item())
print(
    f"[PermeabilityK] field_id={K_ASSET_REPORT['field']['field_id']} | "
    f"sha256={K_ASSET_REPORT['field']['sha256']} | "
    f"K[min,max]=[{_K_GRID_MIN:.6e},{_K_GRID_MAX:.6e}] m2 | k_ref={k_ref:.6e} m2"
)
...
# Apply the shared physical-time window before EOS fitting, time buckets,
# supervised sampling, pair construction, or any nondimensional tensors.
arrays, TIME_CROP_REPORT = crop_row_aligned_arrays(arrays, ANALYSIS_T_MAX_S)
N_rows = int(TIME_CROP_REPORT.rows_after)
time_unique = np.unique(_to_numpy_float64_1d(arrays["t"]))
validate_frozen_crop_report(TIME_CROP_REPORT, int(time_unique.size))
print(
    f"[TimeCropK] cutoff={ANALYSIS_T_MAX_S:.12f} s | "
    f"rows={TIME_CROP_REPORT.rows_after}/{TIME_CROP_REPORT.rows_before} | "
    f"removed={TIME_CROP_REPORT.rows_removed} | "
    f"retained=[{TIME_CROP_REPORT.first_retained_time_s:.6e},"
    f"{TIME_CROP_REPORT.last_retained_time_s:.6e}] s | "
    f"unique_times={time_unique.size}"
)


def _configure_time_reference_from_loaded_data():
    """Keep the frozen H time scale after validating the cropped K window."""
    ...
    if t_max_phys > ANALYSIS_T_MAX_S:
        raise ValueError(
            f"Configuration K time crop failed: retained max time {t_max_phys:.12g} "
            f"exceeds {ANALYSIS_T_MAX_S:.12g} s."
        )
    T_ref = float(T_REF_S)
    ...
    print(
        f"[TimeRef] frozen_to_configuration_H=True | "
...
# -----------------------------------------------------------------------------
# Frozen EOS (rho_co2(p)) fitting
# -----------------------------------------------------------------------------
EOS_RECORD = fit_eos_record(
    arrays["p"],
    arrays["rho_c"],
    reference_dataset_sha256=K_ASSET_REPORT["dataset"]["sha256"],
)
rho_co2_model, rho_c_ref = torch_eos_from_record(
    EOS_RECORD,
    dtype=DTYPE,
    device=DEVICE,
    expected_dataset_sha256=K_ASSET_REPORT["dataset"]["sha256"],
)
print(
    f"[EOS] frozen record={EOS_RECORD['record_sha256']} | "
    f"Cheb deg={EOS_RECORD['model']['selected_degree']} | "
    f"max_rel={EOS_RECORD['fit_diagnostics']['max_relative_error']:.3e} | "
    f"rms_rel={EOS_RECORD['fit_diagnostics']['rms_relative_error']:.3e} | "
    f"range=[{EOS_RECORD['model']['pressure_min_pa']/1e6:.3f},"
    f"{EOS_RECORD['model']['pressure_max_pa']/1e6:.3f}] MPa"
)
...
def pde_residual(model, x_t, y_t, t_t, eps=0.0):
    ...
    vwx, vwy = darcy_velocity_components(
        PERM_FIELD, x_t, y_t,
        relative_permeability=krw, dpdx=dpdx, dpdy=dpdy,
        viscosity_ratio=mu_ref / mu_w, k_ref=k_ref,
    )
    vcx, vcy = darcy_velocity_components(
        PERM_FIELD, x_t, y_t,
        relative_permeability=krc, dpdx=dpdx, dpdy=dpdy,
        viscosity_ratio=mu_ref / mu_c, k_ref=k_ref,
    )
...
requested_checkpoint_dir = os.environ.get("PINN_CKPT_DIR", CHECKPOINT_DIR).strip() or CHECKPOINT_DIR
if Path(requested_checkpoint_dir).resolve() != Path(CHECKPOINT_DIR).resolve():
    raise ValueError("PINN_CKPT_DIR must equal PINN_RUN_DIR for isolated Configuration K artifacts.")
...
# Frozen Configuration H control-volume time step.  It is intentionally not
# re-estimated from K output cadence, because that would alter the M7 loss.
FV_DT_TILDE = float(K_FV_DT_TILDE)
...
def resolved_checkpoint_config():
    runtime_evidence = {
        ...
        "rar_start_iteration": int(RAR_START),
        "rar_update_interval": int(RAR_EVERY),
        "rar_candidate_count": int(RUNTIME["N_CAND"]),
        "rar_residual_select_count": int(RUNTIME["N_SEL_R"]),
        "rar_global_select_count": int(RUNTIME["N_SEL_G"]),
        "python_version": platform.python_version(),
        "torch_version": str(torch.__version__),
        "cuda_version": str(torch.version.cuda),
        "gpu_model": str(RUNTIME.get("gpu_name", "CPU")),
        "slurm_job_id": os.environ.get("SLURM_JOB_ID"),
        "training_script_sha256": file_sha256(Path(__file__).resolve()),
    }
    config = build_resolved_config(TASK_SPEC, K_ASSET_REPORT, runtime_evidence, run_kind=RUN_KIND)
    config.update({
        "run_dir": str(RUN_DIR.resolve()),
        "checkpoint_dir": str(Path(CHECKPOINT_DIR).resolve()),
        "data_time_crop": TIME_CROP_REPORT.as_dict(),
        "retained_unique_time_count": int(np.asarray(time_unique).size),
        "permeability_min_m2": float(_K_GRID_MIN),
        "permeability_max_m2": float(_K_GRID_MAX),
        "reference_permeability_scale_m2": float(k_ref),
    })
    return config


def build_ckpt_config():
    config = resolved_checkpoint_config()
    config.update({
        "exp_name": EXP_NAME,
        ...
        "finite_check_interval": int(FINITE_CHECK_EVERY),
        ...
        "m6_rar": {
            ...
            "n_select_global": int(RUNTIME["N_SEL_G"]),
        ...
        "physical_parameters": {
            ...
            "T_ref": float(T_ref),
            "K": float(K),
            "K_role": "nondimensional reference only; pointwise physics uses permeability_field",
            "permeability_field_id": str(K_ASSET_REPORT["field"]["field_id"]),
            "permeability_field_sha256": str(K_ASSET_REPORT["field"]["sha256"]),
            "pc_enable": bool(PC_ENABLE),
            "pc_entry_pressure": float(PC_ENTRY_PRESSURE),
            "phi": float(phi),
            "rho_w_const": float(rho_w_const),
            "S_ic_co2": float(S_IC_CO2),
            "S_inj_co2": float(S_INJ_CO2),
            "mu_w": float(mu_w),
            "mu_c": float(mu_c),
            ...
            "P_ref": float(P_ref),
            "A_time": float(A_time),
            "domain_x_m": [0.0, float(L_ref)],
            "domain_y_m": [0.0, float(L_ref)],
            "r_well": float(r_well),
            "inj_center": [float(inj_center[0]), float(inj_center[1])],
            "out_center": [float(out_center[0]), float(out_center[1])],
            "Sw_irr": float(Sw_irr),
            "Snr": float(Snr),
...
def save_checkpoint(model, suffix: str):
    ...
    if CHECKPOINT_TAG:
        raise ValueError("Configuration K does not permit checkpoint tags that alter the registered run identity.")
    path, evaluator_alias = checkpoint_paths(
        CHECKPOINT_DIR, TASK_SPEC, profile=RUNTIME["profile"], suffix=suffix
    )
    ckpt = {
    ...
    if evaluator_alias != path:
        if evaluator_alias.is_symlink() or evaluator_alias.exists():
            evaluator_alias.unlink()
        evaluator_alias.symlink_to(path.name)
    print(f"[checkpoint] saved: {path}")
    if evaluator_alias != path:
        print(f"[checkpoint] evaluator alias: {evaluator_alias} -> {path.name}")
    return str(path)
...
def train():
    ...
    for it in range(1, MAX_ADAM_ITERS + 1):
        ...
        check_finite_now = (
            it == 1 or it == MAX_ADAM_ITERS or it % int(FINITE_CHECK_EVERY) == 0
        )
        ...
        if check_finite_now:
            ensure_finite_training_loss(loss, iteration=it)
        loss.backward()
        if check_finite_now:
            ensure_finite_gradients(model.named_parameters(), iteration=it)
        opt.step()
        if check_finite_now:
            ensure_finite_parameters(model.named_parameters(), iteration=it)
    ...
    final_checkpoint_path = save_checkpoint(model, "final")
...
if __name__ == "__main__":
    ...
    write_json_atomic(RUN_DIR / "resolved_config.json", build_ckpt_config())
    _run_started = time.monotonic()
    try:
        _, _final_checkpoint_path = train()
    except BaseException as exc:
        _elapsed = time.monotonic() - _run_started
        _peak_alloc = int(torch.cuda.max_memory_allocated()) if DEVICE.type == "cuda" else 0
        _peak_reserved = int(torch.cuda.max_memory_reserved()) if DEVICE.type == "cuda" else 0
        write_json_atomic(
            RUN_DIR / "training_summary.json",
            finalize_training_summary(
                status="failed",
                elapsed_seconds=_elapsed,
                peak_gpu_memory_allocated_bytes=_peak_alloc,
                peak_gpu_memory_reserved_bytes=_peak_reserved,
        ...
        write_json_atomic(
            ...
            finalize_training_summary(
                ...
                peak_gpu_memory_allocated_bytes=_peak_alloc,
                peak_gpu_memory_reserved_bytes=_peak_reserved,
                checkpoint_path=str(Path(_final_checkpoint_path).resolve()),
                error=None,
            ),
        )
