...
from configuration_k import ANALYSIS_T_MAX_S, DOMAIN_X_M, DOMAIN_Y_M, crop_row_aligned_arrays
...
def default_cfg() -> dict[str, Any]:
    return dict(
        ...
        domain_x_m=(0.0, DEFAULT_L_REF), domain_y_m=(0.0, DEFAULT_L_REF),
        r_well=0.5, inj_center=(0.0, 0.0), out_center=(DEFAULT_L_REF, DEFAULT_L_REF),
...
def cfg_update_from_ckpt(cfg: dict[str, Any], ckpt_cfg: dict[str, Any] | None):
    ...
    if isinstance(phys, dict):
        for src, dst in {
            ...
            "domain_x_m": "domain_x_m", "domain_y_m": "domain_y_m",
...
# =============================================================================
def load_dataset_pack(data_path: str | Path, analysis_t_max_s: float = ANALYSIS_T_MAX_S):
    ...
    arrays, _crop_report = crop_row_aligned_arrays(pack["arrays"], cutoff_s=analysis_t_max_s)
    for k in ("x", "y", "t", "p", "Sco2"):
        if k not in arrays:
            raise ValueError(f"Dataset missing required array: {k}")
        values = to_np_1d(arrays[k])
        if values.size == 0 or not np.isfinite(values).all():
            raise ValueError(f"Dataset required array {k!r} must be nonempty and finite after time cropping.")
    # Always recompute after cropping; pack['time_unique'] still contains K-only
    # layers beyond the H/K common analysis window.
...
def predict_model(model: nn.Module, x_m: np.ndarray, y_m: np.ndarray, t_s: np.ndarray, cfg: dict[str, Any], device: torch.device, batch_size: int):
    ...
    for i0 in range(0, N, batch_size):
        ...
        if not bool(torch.isfinite(pt).all().item()) or not bool(torch.isfinite(ss).all().item()):
            raise FloatingPointError(
                f"Nonfinite model prediction detected for evaluation rows {i0}:{i1}."
            )
