...
S_LIM = (0.0, 0.8)
DS_LIM = 0.25          # symmetric ΔS limits, identical across rows
DP_LIM = 0.35           # MPa
...
for i, r in enumerate(rows):
    ...
    cb1 = fig.colorbar(ims[1], cax=cax1); cb2 = fig.colorbar(ims[2], cax=cax2)
    for cb in (cb1, cb2):
...
import csv
with open(SRC / "FigureB4_manifest.csv", "w", newline="") as f:
    w = csv.DictWriter(f, fieldnames=list(manifest[0]))
    w.writeheader(); w.writerows(manifest)

fig.text(.5,.006,"Seed 0; signed error = prediction − reference. Full-range signed-error scales; no displayed values exceed the limits.",ha="center",fontsize=6.4)
save(fig,"FigureB4")
